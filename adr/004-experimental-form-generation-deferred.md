...
## Decision
Programmatic form generation and cross-account form copying are deferred. The experimental code (`FORMCONFIRMATIONMESSAGE.js`, `formManager.js`) remains in the repository but is not called from any production path. New regions must manually copy the form on first setup.

## Consequences
- `FORMCONFIRMATIONMESSAGE.js` and `formManager.js` are dead code. They must not be invoked in production.
