...
function listFormulas() {
  ...
  for (let i = 0; i < formulas.length; i++) {
    for (let j = 0; j < formulas[i].length; j++) {
      if (formulas[i][j] != "") {
        list.push(`R${i+1}C${j+1}: ${formulas[i][j]}`);
      }
    }
  }

  const outputSheet = SpreadsheetApp.getActiveSpreadsheet().insertSheet('Formulas List');
  outputSheet.getRange(1, 1, list.length, 1).setValues(list.map(f => [f]));
}
