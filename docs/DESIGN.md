...
| Client telemetry (F3Go30-xyri) can't distinguish a request timeout/abort from an immediate network-level reject, or capture retry count / pre-failure HTTP status | Every transport failure funnels through the static page's single `makeTransportError_` path with one shared message — root-causing a client error from Axiom alone is coarser than the issue's original addendum wanted | Known gap, documented on the closed bd issue; candidate fast-follow if the data proves too coarse in practice |
| `tools/build-static-pages.js`'s CSP script-src hashing (`collectScriptHashes_`) must replace any embedded NUL (U+0000) with U+FFFD before hashing | The HTML5 spec's input-stream preprocessing does that same replacement before a browser's parser ever sees the bytes; hashing the raw byte (as `announceFingerprint_`'s field delimiter needs, since it genuinely is one) produces a hash that can never match what the browser computes — silently and permanently blocking that script under the CSP | Fixed and regression-tested (F3Go30-ah3v); `bd memories ah3v-csp-nul-hash` has the full writeup — the same class of bug can recur if a future inline script gains a new byte the HTML5 spec rewrites |
