...
- The static front end is also faster to first paint than the GAS `HtmlService` sandbox
  ...
  instantly instead of blocking on the identify round trip; the live response then reconciles on
  arrival, with any check-in the PAX made this same pageview winning over it, and a revoked/stale
  token clearing the stale snapshot and falling back to the blank identify form (F3Go30-5nfj.5)
...
| Static origin | The GitHub Pages host serving `static-pages/src/index.html` — since ADR-019 the primary front end for every PAX-facing flow. It calls the Apps Script deployment as a JSON API; the deployment's URL rides along in the page URL's `?webapp=` parameter |
