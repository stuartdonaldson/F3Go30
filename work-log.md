...
### Objective 1: Diagnose live cache-wipe-vs-patch behavior and self-heal the roster index after a wipe (C11)
Rationale: After the prior objective added f3Name/range to onEdit cache log lines, the user asked why a specific Celtic edit still wiped instead of patched. Traced the live Axiom event history for that sheetId and found the roster index (the safety guard tryPatchSinglePaxRow_te_ checks) was cold — a wipe only *deletes* the roster index, it never rebuilds it, and rebuilding only happens as a side effect of a live PAX check-in/dashboard read. With no PAX actively checking in on that tracker, every subsequent edit — even a clean single-cell one — kept missing the guard and re-wiping. The user pushed back that "nobody has been inserting rows" (correct — it wasn't a row-insert scenario) and then proposed self-healing the roster index at wipe time, since the trigger already has the sheet open. Refined to rebuilding *only* the roster index via a cheap single-column read, not the full per-PAX row cache (the expensive path only a live dashboard read needs) — a missing per-PAX row cache entry is already handled gracefully elsewhere in the patch path.
Outcome [developer-facing]: PaxCache.js gained `buildRosterIndexFromNames_` (shared roster-index builder, exported); TrackerEditTrigger.js gained `resolveNameColumn_te_` (extracted, shared by patch and rebuild paths) and `rebuildRosterIndexAfterWipe_te_`, wired into `handleTrackerEdit_` immediately after every wipe. New unit test proves a cold-cache wipe self-heals the roster index and the very next edit on the same row patches instead of re-wiping. Full `npm test` suite passes.
Outcome [internal]: Deployed to SIT twice (v2.4.4.2 diagnostic deploy, v2.4.4.3 with the C11 fix); flagged pre-existing duplicated roster-building logic in dashboardWebapp.js (two near-identical loops) as out of scope for this session.

### Objective 2: Extend and fix the live Playwright onEdit trigger spec  [accreted]
Transition: picked up immediately after the self-heal fix, when the user asked whether the existing Playwright suite already covered the various onEdit scenarios — investigating that led straight into fixing what was found broken, while the live-SIT diagnostic context (Axiom queries, sheetId, PAX test identities) was still warm.
Rationale: tracker-edit-trigger-live-check.spec.js was stale (told the operator to check Axiom for `.invalidated` on a scenario that now patches under C10) and covered only one Tracker single-cell edit with no real assertion (`expect(true)`). Extended it to also cover the C11 self-heal round-trip and a multi-cell wipe, while explicitly declining to fabricate untested pixel coordinates for Responses/Bonus Tracker/header-row edits — a wrong guess on a live header cell risks corrupting a column label other code depends on — leaving those branches to the unit suite instead.
...
## 2026-07-26 20:15:00
_session 108de98b-479e-470a-8a08-11da5cce0413 · v3 · 07-26_

### Objective 1: Analyze PROD Axiom logs for errors and checkin identity-resolution cache performance
Rationale: PROD had been live for several days; user wanted to know if there were issues to look at, "In particular I am still keen on performance," specifically how often the checkin page is served from the PropertiesService/CacheService cache versus needing to open the tracker spreadsheet, and why.
Outcome [internal]: No errors/warnings found in PROD logs over 5 days. Of 226 `checkinWebapp.resolveIdentity.timing` events, ~68% were served cache-only (no `SpreadsheetApp.openById`); of the ~32% that opened, 27 (12%) do so unconditionally by design (the checkin-submit handle fast-path must write live), the rest split between genuine PaxCache row misses and — the key finding — the Tracker/Responses layout `CacheService` cache (6h TTL, no write-through) forcing an open purely to re-read day-column headers even when all per-PAX data was still warm.

### Objective 2: Fix the layout-cache invalidation gap  [accreted]
Transition: user pushed back on the layout-cache TTL design ("the only thing that should expire it is structural changes... you could have the sheet structure updated after manual edits just in case"), which led to discovering the onEdit trigger already fires on structural edits but its wipe helper never actually cleared the two layout cache keys — a real, fixable gap rather than an inherent CacheService-vs-PropertiesService tradeoff. User confirmed: "re 3, agreed, fix the gap."
Rationale: `wipePaxCacheAndRelatedCachesForSheet_` (PaxCache.js) and `refreshPaxCacheForSheet_` (markMinusOne.js's nightly repopulate) both already clear/refresh PaxCache rows and full-roster CacheService blobs for a sheet, but never included `go30dash:trackerLayout:`/`go30dash:responsesLayout:` — so a manual structural edit or the nightly backstop left the layout cache riding out its full 6h TTL regardless. Also found the admin `invalidateAllCache` action (WebApp.js) had the same miss for the Responses layout key specifically.
Rejected: batching checkin-submit writes into Script Properties and flushing via the daily markMinusOne/nag triggers, proposed by the user as a related idea — rejected on volume (~5-6 handle-submits/day, not a hot path) and correctness grounds (the Tracker sheet is the live source of truth for TrackerEditTrigger, markMinusOne, nag, and human viewers; deferring writes would create a multi-hour staleness window and conflict with the onEdit staleness guards). User agreed: "ok fair enough."
Outcome [developer-facing]: Added layout-cache clearing to `wipePaxCacheAndRelatedCachesForSheet_` (PaxCache.js), `refreshPaxCacheForSheet_` (markMinusOne.js), and `invalidateAllCache` (WebApp.js, added the missing responsesLayoutCacheKey_). Added test coverage in test_tracker_edit_trigger.js (header-row edit now asserts both layout keys are actually removed from the fake cache) and test_mark_minus_one.js (nightly refresh clears both keys for both months). Full test suite passes.

### Key Learnings:
Axiom dataset field `target` (top-level under `data`, but must be queried as `target` not `data.target` — APL rejects the `data.` prefix even though the raw JSON nests fields under `data`) distinguishes PROD (`TEMPLATE`) from SIT (`TEST`) runs in this project's shared Axiom dataset.
## 2026-07-27 11:52:10
_session 9f09f446 · v3 · 07-27_

### Objective 1: Surface stale installed clients — update-available banner + honest version footer (F3Go30-833s.18)
Rationale: An installed PWA is resumed, not navigated to (Android resumes the task; iOS suspends/resumes for days), so the document may never be re-fetched — a PAX can run a week-old build against a current server indefinitely. 833s.15 had given us detection (clientVersion in the request log) but no remediation. The footer made it worse: it painted the SERVER's appVersion over the build stamp, so a stale client displayed the CURRENT version and the support question "what does the footer say?" was guaranteed to mislead. No new plumbing was needed — the authoritative version already arrives on every identify as `config.appVersion`. The user's framing set the approach: "making sure to prioritize refactoring and reuse rather than prolification of similar solutions."
Rejected: adding a second banner implementation alongside the existing F3Go30-ubwl.3 gas-moved advisory, and a second Playwright/Node fake-DOM harness alongside the existing one — both were near-identical in shape and would have been the exact duplication the instruction warned against. Also rejected my own first test expectation (footer collapsing to `v2.4.7` when client and server agree): keeping `(build)` avoids the footer text visibly changing on the first identify, which would make a *current* client look like it just updated.
Outcome [user-facing]: A dismissible "newer version available — reload" banner (`#updateBanner`) appears when the running build differs from the server's version; the footer now always reports the build actually running, naming the server's version alongside it only on skew. Dismissal is recorded as the version dismissed, not a boolean, so a later release prompts again.
Outcome [developer-facing]: Consolidated rather than added — both page banners now share one `makeDismissibleBanner_` and one `.page-banner` CSS class (the gas-moved block was refactored onto it), and one `applyVersionState_` serves both the load-time paint and `applyServerConfig_`'s repaint, replacing the two hand-written footer assignments whose drift caused the masking defect. Extended the existing `makeGasMovedHarness_` into `makeBannerHarness_` (9 new assertions in test_static_page_client_invariants.js) instead of writing a second harness.

### Objective 2: Document the API backward-compatibility constraint installed clients impose (F3Go30-833s.19)
Rationale: Installing changes a release rule that has been implicit and free until now. `npm run deploy:*` pushes the GAS backend and the static build together, but they land on the client at different times, so once PAX have installed we no longer control when a client updates and the version-skew window is open-ended. Documentation of an existing constraint, not new code — it belongs where it is read at deploy time rather than discovered at breakage time.
Outcome [developer-facing]: docs/OPERATIONS.md gained §API compatibility with installed clients (four rules + why), naming handleCheckinPost_/handleSignupPost_ as the entire blast radius and `clientVersion` in the WebApp.js request log as the way to see which builds are still live before making a breaking change. Cross-referenced from §Installation and from CLAUDE.md §Deploying.
Outcome [internal]: /doc-trigger-check surfaced two further stale targets, both updated — DESIGN.md (the 833s.18 decision entry, including why the banner sharing is deliberate) and docs/CHANGELOG.md §Unreleased (the banner is PAX-visible).

### Objective 3: Deploy to SIT and regression-test, including Playwright  [accreted]
Transition: user instruction after the two beads closed — "deploy to sit and regression test including playwright."
Rationale: The change is client-side and version-comparison-driven, so it could only be trusted after running against a real deployment. Two early full-file Playwright runs failed (1, then 4 tests, all on `#step-checkin` not painting), which had to be resolved to a verdict rather than waved off: the first evidence (passing in isolation) suggested a flake, but a full-file baseline with the change reverted came back 14/14, which pointed at my change. Re-running the full file *with* the change under the same warm conditions was also 14/14 — the confound was ordering, not code. The failing runs were immediately after the deploy wiped 44 PaxCache entries; the baseline ran later against a warm SIT. The isolated-block comparison run first was the weaker evidence, since neither arm was cold.
Outcome [user-facing]: SIT deployed at v2.4.7.3 (GAS push + named deployment + static publish to GitHub Pages, which lagged the push by ~30s before serving the new build).
Outcome [developer-facing]: Added a live Playwright block for 833s.18 (tests/playwright/static-checkin.spec.js) — the existing specs serve unbuilt src/, where a null build stamp correctly suppresses the banner, so nothing exercised the feature live. The new block serves the BUILT dist/sit/ page and stubs only `config.appVersion` to manufacture the skew, covering banner-on-skew, no-banner-when-current, Reload genuinely re-fetching the document (asserted by counting document requests), and per-version dismissal with a later version re-prompting. Reuse held here too: `startStaticServer()` took an optional dir rather than a second server helper being written.
Outcome [internal]: Full green — npm test (43 files), static-checkin 18/18, static-signup 3/3, identity-token-flow 7/7. Confirmed server APP_VERSION and STATIC_BUILD_VERSION_ both derive from the same package.json counter, so a paired deploy always matches and cannot produce a false banner.
Open: nothing committed; F3Go30-833s.20 (human review of the upgrade scenario and install/recoverability gotchas) remains the open gate. test-results/ holds artifacts from the failed runs and is worth clearing.

### Key Learnings:
A GAS deploy's cache invalidation (`invalidateAllCache`, 44 PaxCache entries + 8 layouts here) leaves SIT cold enough that live Playwright specs with 15s waits fail intermittently for several minutes afterward. Let SIT warm before treating a post-deploy failure as a regression — and when comparing against a baseline, run both arms under the same warm/cold conditions or the comparison inverts.

## 2026-07-27 12:32:08
_session a25d6506 · v3 · 07-27_

### Objective 1: Fix a check-in that the spreadsheet recorded but the dashboard did not
Rationale: Reported live from SIT — Little John hadn't checked in for two days, clicked "I hit it" for today and then for yesterday "before the Today one had completed", and got a check-in page showing both days hit against a dashboard showing yesterday missing. The decisive detail came from the developer: "I'll point out the spreadsheet is correct, showing both. It's the dashboard which is stale" — which rules out the sheet write and puts the defect squarely in the derived copy. Root cause is that `handleCheckinSubmit_` built its PaxCache entry from `target.row`, the row as snapshotted when the request resolved identity, patched its own column in, and stored that: two overlapping check-ins for the same PAX start from the same pre-write row, and whichever writes the cache last drops the other's day. The sheet is untouched by the race (different cells), so the corruption is invisible there and permanent in the cache — `buildTrackerValuesFromPaxCache_` keeps serving the row as a hit because the roster index stays complete. Framing the fix as "add a lock" was wrong and the developer pushed on exactly that: "is the lock guard the right place? ... because part of this seems like we have two different copies of the same information and one is stale". Correct statement of the defect is that a cache entry derived from pre-write state is wrong regardless of locking; deriving it from the sheet *after* the write makes it right by construction, and the lock is then only a hardening detail closing the residual ordering window (re-read + cache write held together, cell write deliberately left outside so a morning check-in burst isn't serialized script-wide).
Rejected: consolidating the browser's duplicate copies as *the* fix. It would have made the two views agree in this incident — agree on the client's optimistic state, while the server's cached row stayed corrupt for every other device, every cold load, and every other PAX viewing the board. Filed as its own issue (F3Go30-os03) sequenced explicitly after the server fix, since doing it first masks the defect.
Outcome [developer-facing]: New shared `refreshPaxCacheRowFromSheet_` in `script/PaxCache.js` (flush → re-read the PAX's single row post-write → cache it, lock-guarded, entry deleted rather than written if the lock can't be taken). `handleCheckinSubmit_` calls it; the cross-month branch of `resolveCheckinDayTarget_` gets it through the same call rather than a second hand-copied implementation.
Outcome [user-facing]: Concurrent same-PAX check-ins no longer clobber each other on the dashboard; a PAX's own score column also stops lagging their own check-in.
Outcome [developer-facing]: Four tests in `test_dashboard_webapp.js`, written first and confirmed RED — the lost-update one failed reproducing the exact report. Two fixture changes were required to express the behaviour at all: the fake tracker sheet now *applies* writes to its backing rows (it previously only recorded them, so a post-write re-read was untestable), and gained a one-shot `_afterNextRowRead` hook that simulates a concurrent execution's write landing between this request's read and its cache write.
Outcome [developer-facing]: `docs/DESIGN.md` §Caching — cache-table row 1 invalidation cell plus a paragraph stating the post-write derivation rule, since ADR-016's write-through model is documented there and the old text implied the snapshot patch was sound.
Outcome [internal]: F3Go30-xg8f filed and claimed (P1); F3Go30-os03 filed for the client single-copy consolidation.
Open: AC5, the live SIT round trip reproducing the overlapping clicks against a warm dashboard, still needs a deploy — nothing deployed and nothing committed. F3Go30-s1a5 turned out to specify this exact fix shape for a different symptom of the same root cause and is now partially resolved; its item 2 remains open — `bonusWebapp.js` `handleBonusAdd_`/`handleBonusEdit_` never touch cached tracker rows at all, so a bonus write still leaves the cached score stale. The helper is now in place, so wiring those paths should be small.

### Key Learnings:
A GAS cache entry patched from a request's own pre-write snapshot has two independent failure modes that look unrelated in the field — a lost update under concurrency, and stale sheet-formula columns (Score/Raw Score/bonus totals) — and both were filed separately before the shared cause was spotted. `SpreadsheetApp.flush()` before the re-read is what makes the recalculated formula values readable back in the same execution.

## 2026-07-27 13:58:00
_session c74c96c4 · v3 · 07-27_

### Objective 1: Close out F3Go30-xg8f — deploy the post-write cache derivation to SIT and reproduce the original lost update live (AC5)
Rationale: The fix and its unit tests were already written and RED-verified in the prior session, but AC5 — "live SIT round trip reproducing the original report" — cannot be satisfied by a fake sheet: the defect only exists because two real GAS executions can interleave, and a green Node test proves nothing about whether they actually do. The prior session's own key learning also warned that a post-deploy SIT is cold enough to make live specs fail spuriously, so evidence had to be strong enough to distinguish a real pass from a warm/cold artifact.
Rejected: trusting the client-side wall clock as proof of overlap. Two POSTs fired with `Promise.all` finishing in 5.8s (versus 6.3s + 4.9s sequential) shows the *requests* overlapped, not that both executions resolved identity before either wrote — which is the only interleaving that reproduces the bug. Cross-checked the Axiom event timeline per execId instead: `resolveIdentity.timing` for both executions (15.372s, 15.903s) landed before either `checkinWebapp.checkin` write (16.532s, 17.477s), on both runs.
Outcome [user-facing]: SIT deployed at v2.4.7.4; overlapping same-PAX check-ins verified to leave both days in the warm PaxCache-served view (monthGrid), sheet cross-checked at Jul26=1/Jul27=1/Score=10.
Outcome [internal]: F3Go30-xg8f closed with the Axiom execId timeline recorded on the issue as the overlap evidence. Live repro kept as a scratchpad script; `fetch` could not reach script.google.com from this sandbox, so it reuses callWebapp.js's `https` + manual-302-follow transport.

### Objective 2: Fix F3Go30-s1a5 item 2 — bonus writes leave the cached tracker row's formula columns stale  [accreted]
Transition: direct user instruction once xg8f was verified — "so lets fix s1a5 before committing." The shared helper had just landed and the SIT/Axiom context was warm, so wiring the remaining write path was cheaper now than cold later.
Rationale: The Tracker's Score / per-type bonus columns are formulas fed by the Bonus Tracker sheet, but `handleBonusAdd_`/`handleBonusEdit_` never touched cached tracker rows at all — so a bonus moved the pills (computed server-side from the bonus-entries cache) while the score kept its pre-bonus value in the same payload. Same root cause as xg8f and the same fix shape, so the standing reuse instruction applied directly: `resolveBonusSheet_` now carries sheetId/targetSs/trackerRowIndex (identity is already resolved there, so no caller pays for a second resolution) and one `refreshTrackerRowAfterBonusWrite_` calls the shared `refreshPaxCacheRowFromSheet_` from all four write sites, including both months of a cross-month edit.
Rejected: two candidate live-verification runs were reported honestly as inconclusive rather than being written up as passes or failures. A Fellowship add moved nothing (already weekly-capped that period) and a second EHing FNG moved the pill but not the score — the latter looked exactly like the bug under test but is a *different* defect, confirmed by reading the sheet: the Tracker formula caps EH per period while `BonusTypes.js` marks it `weeklyCap: false`. Re-ran with Q Point, which had genuine headroom in the period.
Outcome [user-facing]: A bonus add/edit now updates the dashboard's score immediately, in step with its pills; verified live on SIT v2.4.7.5 (score 16→17, q pill 0→1, rawScore correctly unchanged) with the sheet reading Score=17 at the same moment.
Outcome [developer-facing]: 4 new tests in test_dashboard_webapp.js (add, same-month edit, cross-month edit refreshing both rows, lock-unavailable-drops-the-entry), each confirmed RED *individually* against the disabled fix rather than inferring the other three from the first failure. New end-to-end bonus fixture drives the real handlers via a stubbed `resolveTrackerForContextDate` plus Responses/Tracker/Bonus Tracker fakes, with the bonus write triggering a stand-in for the Tracker's own formula recalc. `handleBonusAdd_`/`handleBonusEdit_` exported for test visibility.
Outcome [developer-facing]: DESIGN.md §Caching post-write derivation paragraph now names both writers into a PAX's Tracker row; CHANGELOG §Unreleased gained the combined user-facing bullet.
Outcome [internal]: F3Go30-s1a5 closed; F3Go30-fkox filed (P2) for the EHing FNG cap disagreement — sheet caps per period, BonusTypes.js says uncapped, so pills and score can legitimately disagree and the "does not count" save-time notice won't fire for a repeat EH. Flagged as a program-rules decision, not a code fix.

### Objective 3: Commit four sessions' worth of accumulated work  [accreted]
Transition: the tree had been left uncommitted across the PWA-banner, layout-cache, day-boundary-test and cache-fix sessions; the user chose a split-by-concern commit strategy when asked rather than one omnibus commit.
Rationale: File-level entanglement (PaxCache.js and DESIGN.md each carried two concerns; CHANGELOG carried two independent bullets in one hunk) made a strictly per-session split impractical, so the cut was made by *concern* instead: batched Bonus Tracker writes, the stale-client banner + API-compat docs, and cache-copy correctness (xg8f + s1a5 + the layout-cache invalidation gap they share a theme with). The CHANGELOG hunk was split by staging the file twice.
Outcome [developer-facing]: Three commits (3c5da92, e8543fc, f749351), full suite green after each.
Open: `script/version.js` deliberately left uncommitted — the SIT deploy stamped it `2.4.7.5 / TEST` and committing that would flip the checked-in target away from `TEMPLATE`.
...
## 2026-08-02 22:44:56
_session 6aba5053 · v3 · 08-02_

### Objective 1: Diagnose why teammates' dashboard streaks look month-truncated
Rationale: User noticed team-tile streaks reset each month; investigation traced it to buildDashboardPaxRow_ computing streak/maxStreak30 from current-month-only dayValues, with the cross-month correction (getPriorMonthTailValues_) applied only to the logged-in viewer's own top-level fields, never to myTeam/paxBoard rows.
Outcome [internal]: Root cause identified and explained (dashboardWebapp.js:2208-2260); confirmed not a client-side localStorage issue — no PAX data is cached in browser storage at all.

### Objective 2: Design a PAX-centric data model and contract, document it, and file the first migration slice  [accreted]
Transition: Once the root cause was understood, the user asked to look further ahead — "eventually we will move away from the spreadsheet" — shifting from diagnosing the bug to designing the replacement model, while the contract/caching context was already loaded.
Rationale: Goals already change more than once per tracker-month today (re-save overwrites, no history) and reporting needs point-in-time goal lookups, so goals needed to become a versioned list, not a scalar. History needed one self-describing, month-boundary-agnostic record per PAX instead of per-month Tracker/Responses rows. "that seems like it should be embedded in the record" — user caught that the proposed 400-day history window had no anchor date, corrected to add historyEndDate.
Rejected: A cheaper per-request bulk lookup (batch-fetch every teammate's prior-month tail at read time) was considered and rejected in favor of a rolling f3Name-keyed cache, since the bulk-lookup approach keeps the cache month-scoped and would need to be undone by a later real migration.
Outcome [developer-facing]: Created docs/pax-data-model-and-contract.md — full current-state contract (every cmd=checkin/cmd=signup action), before/after Mermaid diagrams, a per-store data inventory (Spreadsheet/PropertiesService/CacheService/browser), a proposed PaxProfile model (versioned goals list + anchored rolling history string), and a 4-slice migration path.
Outcome [internal]: Filed bd issue F3Go30-5uk2 (Slice 1: f3Name-keyed rolling history window) with description, design, and acceptance criteria, ready to claim.
## 2026-08-03 13:27:49
_session 512cd86f-048f-4bd2-a3ea-99999561da79 · v3 · 08-03_

### Objective 1: Ship a genuinely working cross-month streak/maxStreak30, as Slice 1 of the PaxProfile data-model migration
Rationale: F3Go30-5uk2 (Slice 1 of docs/pax-data-model-and-contract.md's migration) had its server-side implementation already written but unverified; finishing it meant not just confirming the algorithm but making the whole feature actually correct end to end for a real PAX using it. That end-to-end bar surfaced two more gaps in the same slice, not separate problems: (1) the admin "Invalidate Cache" action couldn't clear the new go30hist: rolling-history entries this slice introduced, so a wrong streak had no recovery path — found when the user asked "how do I force the streak cache to reload?"; (2) after fixing that and redeploying, the user hit a real live defect ("Guero's streak is not counting last month, and neither is Pogo") that traced not to the server (an offline replay against real SIT sheet data proved the server algorithm was already correct) but to the browser: `memberViewForIndex_` in both static-pages/src/index.html and script/CheckinApp.html was discarding the server's correct historyValues-derived streak and recomputing a same-month-only approximation for every myTeam/paxBoard tile — leftover client logic from before this slice shipped server support, with a code comment that had documented the truncation as intentional. All three fixes were required before the slice could be called done, not accretion onto a finished task.
Rejected: none — each fix was the necessary next layer once the previous one exposed it, not an alternative approach.
Outcome [user-facing]: Team-tile and self streak/maxStreak30 now correctly span a month boundary for every PAX viewing "today," not just the logged-in viewer; a stale/wrong streak can be force-reloaded via "Invalidate Cache."
Outcome [developer-facing]: wipeAllPaxCache_ now clears go30hist: entries (test_pax_cache.js); added test/test_client_streak_display.js (extraction-test pattern, both client copies); F3Go30-5uk2 closed with full verification notes. Deployed and live-verified on SIT across three iterations (v2.4.10.3 → .5); final state confirmed against an independent offline replay of Güéŕó's (9) and Pogo's (3) real July+August data.

### Objective 2: Scope the next slice — same architectural gap in rollingAverage
Rationale: User asked where rollingAverage is calculated and whether it's redundant given go30hist now exists, after noticing Pogo's 7-day chart looked incomplete — the same viewer-only-override shape as the streak bug, just never touched by Slice 1 (buildDashboardPaxRow_ computes rollingAverage same-month-only for every row; only the viewer gets a getPriorMonthTailValues_-corrected version). User asked to capture it in a bead and check for other similar redundant-call patterns before moving on.
Outcome [internal]: Filed F3Go30-uz9e.1 (description/design/AC, including a noted efficiency side-effect: getPriorMonthTailValues_ currently runs twice for the viewer in one cold-cache request), linked discovered-from F3Go30-5uk2. A broader codebase scan (subagent) found no other instances of the same redundancy pattern — this is the last piece of Slice 1's architecture left to extend.

### Key Learnings:
SIT is not an isolated test environment for this project — it carries real concurrent traffic from the actual developer's own F3 persona ("Little John"), which showed up in Axiom logs mid-investigation and had to be distinguished from genuine bugs before concluding anything from live data.
## 2026-08-03 15:29:53
_session 0c98c89a · v3 · 08-03_

### Objective 1: Fix F3Go30-uz9e.1 — rollingAverage's month-boundary bug for myTeam/paxBoard rows
Rationale: rollingAverage (dashboardWebapp.js) had the same month-boundary truncation bug streak/maxStreak30 had before F3Go30-5uk2 — buildDashboardPaxRow_ built it from dayValues (this month only) for every roster row, with a separate viewer-only override (buildRollingAverageWithLookback_ + a dedicated getPriorMonthTailValues_ call in handleCheckinDashboard_) papering over it just for the logged-in PAX. Checked for other open issues touching the same code path (csfe, os03) — neither is a month-boundary bug, so this stayed scoped to uz9e.1 alone. Applied the same consolidation 5uk2 already used for streak/maxStreak30: source rollingAverage from the already-computed historyValues (PaxCache's f3Name-keyed rolling window) for every row, sliced back to dayValues' own length to keep the response shape unchanged.
Rejected: kept buildRollingAverageWithLookback_ as a standalone function initially, but once its only call site (the viewer-only override) was retired it became dead code (only reachable from tests) — deleted it and its 3 dedicated unit tests rather than leaving unused code, per the project's no-dead-code convention; replaced with tests against buildDashboardPaxRow_'s new historyValues-sourced behavior instead.
Outcome [developer-facing]: buildDashboardPaxRow_ now computes rollingAverage from historyValues (falls back to dayValues when absent, matching maxStreak30's existing fallback); handleCheckinDashboard_ retired the separate userRollingAverage override, sourcing it from userRow.rollingAverage instead, and now derives priorMonthDayValues from the already-fetched userHistoryValues rather than a second getPriorMonthTailValues_ call (closing the noted cold-cache double-fetch). Removed buildRollingAverageWithLookback_ (dead after the override's removal) and its export; test_dashboard_webapp.js updated with new buildDashboardPaxRow_ unit tests plus an integration test asserting a teammate's rollingAverage spans the month boundary and the viewer's top-level figures match with no regression. Full npm test suite passes.
Outcome [user-facing]: A PAX's team-tile pax-detail popup 7-day rolling-average chart now shows a real trailing window (padded from the prior month) for every teammate/board row, not just the logged-in viewer — fixes the truncated 1-3-point chart reported live on SIT.
Open: Live SIT verification (reproducing the Pogo/Güéŕó scenario) per this project's TDD-with-live-verification convention has not yet been run — deploy + live check still needed before closing F3Go30-uz9e.1.
## 2026-08-03 18:45:00
_session 80e32014 · v3 · 08-03_

### Objective 1: Investigate GAS-hosted page usage and the `?static=0` fallback for a retirement decision
Rationale: User was "considering whether to retire those entry points" (the GAS-rendered `CheckinApp.html`/`SignupApp.html` fallback path) and wanted evidence, not guesswork, before deciding.
Outcome [internal]: Traced the redirect-first routing architecture (`renderStaticRedirect_`, single-tap interstitial due to GAS sandbox iframe restrictions on scripted top-navigation) and confirmed the pages are still live but demoted to fallback-only per ADR-018/019. Queried Axiom (`tools/query_axiom.py`, using bare field names — `data.`-prefixed fields error with `invalid field: "data"`) and found zero organic PROD usage of `?static=0` in 90 days; all 96 hits were confined to SIT during a test week. Also surfaced (not requested, offered proactively) that all 218 PROD checkin/signup arrivals sampled in that window were synthetic (`runId: 'gaslogger-test'`), so there's no real PAX traffic in the window to validate against either way.
Open: Gap between 218 PROD arrivals and only 20 PROD `staticRedirect` log events was flagged but not chased further (time-boxed, tangential to the core question).

### Objective 2: Git/branch audit, uncommitted-diff summary, and branch cleanup
Rationale: User asked for git/branch status, then "what is uncommitted here? give me a summary by branch of what is on that branch and needs merging" — wanted to understand both the substance of in-progress uncommitted work and whether any of six stale local branches held unmerged content before deleting them.
Rejected: Initial branch comparison used raw commit/line counts (`dashboard-tool` showed +7524/-176 across 69 files vs `main`), which looked like real unmerged work; deeper investigation (file-existence diff at the merge-base, then line-count comparison of `dashboardWebapp.js`/`bonusWebapp.js`/`PaxCache.js` on both branches) showed `main`'s versions were strict supersets — `dashboard-tool`'s feature work had been independently reimplemented and substantially advanced on `main`, so the diff stat alone was misleading and confirmed only after that follow-up.
Outcome [internal]: Confirmed via `work-log.md`'s own diff that the uncommitted working-tree changes are the tail of the cross-month streak/rollingAverage fix (F3Go30-5uk2, F3Go30-uz9e.1), with F3Go30-uz9e.1's live SIT verification still outstanding. Confirmed 5 of 6 local branches (`f3go30-833s-pwa-design`, `feat/checkin-edit-signup-link`, `hardening/identify-once`, `inspiration-and-celebration`, `master`) were fully merged into `main` (0 commits ahead) and `dashboard-tool` was superseded-not-missing.
Outcome [developer-facing]: Extracted the two files unique to `dashboard-tool` with no equivalent on `main` — `Go30-Demo-Script.md` and `tests/playwright/demo-screenshots.spec.js` — into new `docs/needs-review/` before deleting the branch, at user's explicit request ("pull out ... then we can clean out the dashboard-tool branch"). Deleted all 6 stale local branches (`dashboard-tool`, `f3go30-833s-pwa-design`, `feat/checkin-edit-signup-link`, `hardening/identify-once`, `inspiration-and-celebration`, `master`) after confirming each was either fully merged or superseded.
Outcome [internal]: Follow-up question ("didn't inspiration-and-celebration have code on it?") confirmed no work was lost — its tip commit is reachable from `main` and its output (`static-pages/src/inspire/` playground, ADR-017/F3Go30-koqf) is present. Clarified for the user that this playground is explicitly unshipped design work (per its own README) — not bundled by `build-static-pages.js`, not wired to any real check-in/streak/goal trigger.

### Key Learnings:
Axiom APL field references must be bare (`queryString`, not `data.queryString`) even though the JSON response nests fields under `"data"` — a `data.` prefix throws `invalid field: "data"`.

## 2026-08-04 07:29:34
_session badcfad5 · v3 · 08-04_

### Objective 1: Fix F3Go30-uz9e.2 — anchor the go30hist rolling window on read and reconcile it against the Tracker
Rationale: `historyEndDate` had been stamped on every write since 5uk2 but never read back, so the cache-hit path returned whatever the last write left and `buildDashboardPaxRow_` tail-aligned it as though it ended today. Three symptoms, one defect: a pre-marked future day collapsed a live streak to 1 on every team tile (and corrupted the stored window permanently, since the nightly minus-one job only writes EMPTY cells and never rewrites a pre-marked `1`); prior-month date-nav rendered August figures under July's dayValues; and a stale window shifted everything by N. Enforced at two points per the issue's design — anchor on read (four branches, anchored to `dayDates[dayDates.length - 1]`, *not* `reportedCutoff`, which stays at today when navigating backward), and reconcile the overlapping tail against the Tracker row that is fetched regardless. Added the write-side clamp as well rather than leaving it optional: dropping a future-day write kills the front-of-window data loss at source, and reconciliation brings the value back when the day arrives.
Rejected: sending the window over the wire (the issue's own out-of-scope call) — the Tracker read cannot go away while date-nav resolves arbitrary months, so it would add a branch rather than remove one. Deferred to Slice 3.
Outcome [user-facing]: Pre-marking a day ahead no longer wipes out streak/30-day-best on team tiles; date-arrow navigation into an earlier month now computes every figure as of that month's last day; the 7-day chart plots one point per day of the viewed month.
Outcome [developer-facing]: `anchorPaxHistoryValues_` + `getPaxHistoryEntriesBulk_` (one `getProperties()` per dashboard load, not one per roster row) + a future-day clamp on `advancePaxHistoryDay_` in PaxCache.js; `getPaxHistoryWindowValues_` anchors and reconciles, and its rebuild no longer trims trailing blanks (that trim was what made the stamped `historyEndDate` a lie whenever today's cell was empty). `purgeStalePaxCache_` gained a fourth pass reaping go30hist entries — they carry no sheetId, so no existing pass could ever see them. 13 new unit tests; full suite green.
Outcome [developer-facing]: Found live during SIT verification and fixed: `rollingAverage` came back 13 long against 31 `dayDates` on a July view. `historyValues` caps at `MAX_STREAK_WINDOW_DAYS_` (30), shorter than a 31-day month, so averaging over it and slicing produced fewer points than `dayValues` — breaking the 1:1 alignment both server and client assume, and off-by-one even for a 31-day *current* month at month end. `buildDashboardPaxRow_` now averages over the prior-month lead plus the full `dayValues`.
Outcome [internal]: `PAX_HISTORY_WINDOW_DAYS_ = 40` documented as load-bearing beyond maxStreak30 (31 + 13 = 44 > 40 works only because the padding is wanted early in the month). docs/pax-data-model-and-contract.md §Slice 1 gained a "read side, as shipped" section. uz9e.1 raised to P1 and marked blocked-by uz9e.2 — they became one change with one SIT run — then both closed after live verification on SIT @229.
...
## 2026-08-04 16:15:42
_session 9c5f4f4e-7980-4148-888c-5feba01d7b0a · v3 · 08-04_

### Objective 1: Remove the GAS-rendered signup/check-in pages outright (DR-04) and confirm it end-to-end in SIT
Rationale: DR-04 (design-review-2026-08-04.md) found the removal posture already decided (F3Go30-90l5/ADR-019) but scoped to `SignupApp.html` only, with the real duplicate surface (`CheckinApp.html` + `IdentityCore.html`, ~3,000 lines) still live and actively drifting from `static-pages/src/index.html`. The developer chose to skip the remaining soak-period trigger and execute the full removal now rather than extend the bead's scope and keep waiting: "We no longer need the html service gas entry points for the checkin, signup or identitycore pages. clean this up... confirming that full integration test has been done in the SIT environment... minimizing and reducing unnecessary duplication."
Rejected: DR-04's own recommended sequence (extend `F3Go30-wjpu`'s scope, state a concrete trigger date, wait) was explicitly overridden by direct instruction to resolve completely now.
Outcome [developer-facing]: `script/SignupApp.html`, `script/CheckinApp.html`, `script/IdentityCore.html` deleted (3,044 lines); `doGet` now redirects to the static front end unconditionally (no more `?static=0` opt-out or template-rendering fallback); `doPost` JSON API unchanged. Orphaned code removed alongside it: `buildCheckinPageOutput_`, `renderCheckinPageForTypedIdentify_`, the `formIdentify` doPost branch, and the check-in session title-cache mechanism that existed only to serve the removed template's `<title>`.
Outcome [developer-facing]: 4 unit-test files retired outright (coverage already subsumed elsewhere), 7 more repointed at the static page or trimmed of GAS-only assertions; `tests/playwright/identity-token-flow.spec.js` retired (its GAS-iframe coverage was already duplicated against the static front end by `static-checkin.spec.js`/`static-signup.spec.js`); `tools/sync-how-it-works.js` no longer syncs into the two removed files. All 44 `npm test` suites pass.
Outcome [developer-facing]: `adr/021-gas-rendered-pages-removed.md` supersedes ADR-019's `?static=0` claim (passed `/adr-quality-check`); `docs/DESIGN.md`/`OPERATIONS.md`/`pwa-design.md`/`Go30-Intro.md`/`pax-data-model-and-contract.md` updated to stop describing the removed capability as live; `docs/design-review-2026-08-04.md` DR-04 resolved (DR-06 partially resolved, DR-10 closed as direct consequences). `docs/CHANGELOG.md` deliberately left untouched — no PAX-visible symptom, per that file's own inclusion rule.
Outcome [internal]: `F3Go30-wjpu` closed (scope extended and executed same day); `F3Go30-zcxc` closed obsolete.
Outcome [user-facing]: Live-verified in SIT — deployed, full namespace smoke test passed end to end (signup → check-in → dashboard → bonus → cross-month relocation → teardown), old `?cmd=signup`/`?cmd=checkin` links confirmed via direct `curl` to still redirect with every param intact, and a full live-browser Playwright pass against the deployed SIT webapp (`static-checkin` 16/17, `static-signup` 3/3, `checkin-advanced-grid` 17/17, `demo-screenshots` 4/4 — the one flake is a pre-existing route-mock/reload timing issue unrelated to this change).
Open: work is staged but not committed/pushed, pending developer review; whether/when to also cut this to PROD was not addressed (default scope was SIT per project convention).
...
## 2026-08-11 09:20:43
_session 2692397b-7100-44b4-bcfb-2f9964c81cf4 · v3 · 08-11_

### Objective 1: Implement F3Go30-g9bi — Config-driven announcement splash
Rationale: bd issue F3Go30-g9bi called for a Site-Q-pushable splash notice on the check-in app, with a design already brainstormed (Config-sheet source of truth, server-computed day/day+3 validity window, piggybacked on the existing identify/dashboard/checkin config payload, client-only V1 dismissal). Implementation gate run first; two genuinely open design questions (blocking modal vs banner, Dismiss-only vs Dismiss+Remind-Later) resolved with the developer before coding. Shipped with TDD (server window-resolution logic, client show/hide/dismiss/snooze logic) and live-verified against real SIT — not just unit-mocked — closing the issue once confirmed. The rest of the session was the developer catching defects in that shipped implementation through real usage on SIT and directing fixes; each is called out below as a defect-in-the-original rather than a separate objective, per developer instruction: "the real objective was the config-driven announcement. the other fixes were bug fixes in that announcement and really just revisions on it... they are defects in the original implementation."
Outcome [user-facing]: Shipped and live-verified on SIT: a Site Q adds an `Announce.<day>` row to the Config sheet (title + optional HTML body — links/formatting allowed); pax get a blocking modal that auto-expires after day+3, newest-day-wins on overlap, dismissible or snoozable 30 minutes.
Outcome [developer-facing]: `resolveActiveAnnouncement_dw_`/`checkinClientConfig_dw_` (dashboardWebapp.js), client modal + `applyAnnouncementState_` (static-pages/src/index.html), a `setConfigValue` admin action (generic Config-sheet write, for live-testing without hand-editing the sheet) with unit tests, and `tests/playwright/announcement-splash-live-check.spec.js` as permanent real-browser regression coverage.

Defect 1 — blank/inert modal on SIT ("I see the splash... but it is blank... clicking doesn't do anything"): `.hidden{display:none}` and the new `.announce-overlay{display:flex}` were both single-class selectors of equal CSS specificity; declaration order alone decided the winner, so the overlay stayed rendered — full-screen, blocking every click on the page underneath — regardless of the `hidden` class. The classList-only Node test harness could never have caught this; it isn't a real stylesheet. Fixed with a `.announce-overlay.hidden{display:none}` override (specificity, not order, wins) and live-verified with real Playwright `toBeVisible()`/`toBeHidden()` assertions (checks computed layout, not just the class) against SIT.
Defect 2 — Config row shape didn't match the real need: revised from a single message field to title (column B, plain text) + body (column C, HTML — links/formatting), with a blank title explicitly meaning "empty, not active" regardless of what's in the body column, per direct instruction.
Defect 3 — an already-dismissed day silently swallowed a same-day content edit ("I just did an update to Announce.11 and it is not showing up"): dismissal/snooze had been keyed on the day number alone; a Site Q editing an already-dismissed day's message left it permanently suppressed for anyone who'd dismissed the earlier version, with no way to re-surface short of the day rolling over. Rekeyed to a fingerprint of {day, title, message} — mirrors how `updateBanner_` already keys its own dismissal on the server version string, not just "an update happened."
Defect 4 — 3x redundant live Sheets API reads per identify ("We should not have added another call here"): `checkinClientConfig_dw_` opened and read the Config sheet separately for Site Q, NameSpace, and the new announcement lookup. Consolidated to one open+read, shared across all three. Checked Axiom's actual `durationMs` history before/after to report honestly: identify has run 2-8s server-side since before this feature existed, so the redundant reads were real waste now fixed, but not the dominant cause of the perceived slowness — that baseline predates and is unrelated to this feature.
Defect 5 — excessive vertical whitespace in the rendered body (screenshot + exact HTML provided): admin-authored `<p>`/`<ul>`/`<li>` content was getting the browser's default UA-stylesheet spacing stacked on top of the modal's own paragraph margin. Added explicit tight `ul`/`li` rules and switched `#announcementText` from a `<p>` to a `<div>` (block content nested inside a literal `<p>` is invalid HTML, even though browsers happened to render it tolerably via innerHTML fragment parsing).
Defect 6 — the live-check spec itself was unsafe and mis-configured: it ran non-headless (missing the `test.use({headless:true})` override every other non-GAS-editor spec in this repo carries, since the global config defaults to `headless:false` for the GAS-editor specs), and its cleanup unconditionally blanked `Announce.<today's real day>` — which, because the developer was actively using that exact key for a real live announcement, silently clobbered real content on more than one run. Fixed headless; added a read-only `getConfigValue` admin action so the spec captures the row's real original value before overwriting and restores it exactly, never a blind clear.
Outcome [developer-facing]: Added `tools/wait-for-static-deploy.js` (+ unit tests, wired into `npm test`) — GitHub Pages has real CDN propagation lag after a deploy, and this had been improvised as an inline poll loop each time a live check needed to confirm a fresh deploy had actually landed; now a standing, documented tool (docs/OPERATIONS.md, CLAUDE.md).
Outcome [user-facing]: `docs/CHANGELOG.md` Unreleased entry rewritten to the developer's preferred concise wording for the pax-detail-popup and bonus-points entry points.

### Key Learnings:
Every defect here was findable before shipping, not just in principle:
- A `hidden` class combined with an element that sets its own explicit `display` (flex/grid/etc.) needs either a same-file `.selector.hidden{display:none}` override written up front, or a real-browser check (Playwright `toBeVisible`/`toBeHidden`) — a synthetic-DOM/classList-only test harness structurally cannot see a CSS cascade bug, no matter how thorough its assertions look.
- When a feature reads the same Config/data source for multiple values, open and read it once and share the rows — reaching for a per-lookup helper (`getConfigValue_dw_(spreadsheet, key, null)`) that re-opens the sheet each call is an easy trap to fall into incrementally, one lookup at a time, without ever seeing the total request count grow.
- Value-keyed client dismissal needs to key on content, not just an identifier that can be reused with different content behind it (a day number, here) — the moment the thing behind the key can change, the key has to change with it.
- A live-check script that overwrites shared/production-adjacent state for the duration of a test must capture-and-restore the original value, never blank-and-hope — especially once that state ("today's announcement") could plausibly hold real, actively-authored content rather than only ever be test fixtures.

## 2026-08-19 21:13:41
_session 74b14be4-bf4d-4c84-8e03-69554f2d1877 · v3 · 08-19_

### Objective 1: Implement PLAN-08-19.md Stage 1 — client resilience & telemetry (F3Go30-xyri, F3Go30-n40u, F3Go30-ah3v)
Rationale: PLAN-08-19.md sequenced these three issues into one session because all three touch the same static-page transport block/`<head>` — xyri's error-capture pipeline, n40u's background reconnect poll (whose poll attempts/outcomes were designed to feed xyri's stream), and ah3v's CSP/referrer meta, done last so it wouldn't entangle with the transport-logic diff. Implementation gate run first (existing tracked bd issues, drafted AC read and firmed up against the real code before writing any of it). Followed the project's reuse-over-proliferation convention throughout: reused GasLogger's existing Axiom-logging/PII-masking conventions and PaxCache/CheckinSessions' PropertiesService-backed dedupe-store pattern server-side; reused the existing extraction-based test-harness pattern for the client code, adding a shared `test/helpers/staticPageExtract.js` for the new test file rather than hand-copying `readStaticPage_` a fifth time (four pre-existing copies flagged, not retrofitted — out of scope for this session).
Outcome [user-facing]: The "Couldn't reach the server" banner now clears itself automatically once connectivity returns (background poll, bounded to ~5 min), with a manual "Reload page" button offered only past a 2-minute outage — no more forced manual refresh. CHANGELOG.md §Unreleased carries the one PAX-visible bullet.
Outcome [developer-facing]: New `script/ClientTelemetryLog.js` (server-side dedupe + Axiom logging for client-captured errors and reconnect-poll outcomes, wired into `handleCheckinPost_` as action `clientTelemetry`); static-pages/src/index.html's transport block gained a bounded-queue capture/upload pipeline and the reconnect-poll state machine; `tools/build-static-pages.js` now computes real CSP script-src hashes and inserts a `<meta http-equiv="Content-Security-Policy">` at build time (src/ itself stays CSP-free via an inert marker comment, so local/Playwright dev against unbuilt src is unaffected). Test coverage: test/test_client_telemetry_log.js (9 cases), test/test_client_reconnect_telemetry.js (20 cases, new shared harness), test/test_build_static_pages.js (+7 cases), plus a new live Playwright block in tests/playwright/static-checkin.spec.js that serves the *built* SIT page against the real deployed backend and asserts zero CSP violations through both check-in and signup. Deployed to SIT (build 2.5.0.1) and live-verified end to end, including a real `clientTelemetry` POST against the deployed server. All three bd issues closed with detailed implementation comments; PLAN-08-19.md's Stage 1 checklist and docs/DESIGN.md updated.
Open: xyri's captured diagnostic detail can't yet distinguish a request timeout/abort from an immediate network-level reject, or carry retry count/pre-failure HTTP status — every transport failure still funnels through one shared `makeTransportError_` message. Documented as a candidate fast-follow on the closed bd issue and in PLAN-08-19.md rather than silently dropped; not pursued this session to avoid scope creep on top of three issues already landing together.

### Key Learnings:
- A hash-only CSP's `script-src` hash must be computed against what the browser's HTML parser actually sees, not the raw source bytes: the HTML5 spec's input-stream preprocessing silently rewrites any literal NUL (U+0000) to U+FFFD before the parser (and therefore the browser's own CSP hash check) ever runs — a script containing an embedded NUL (found live here: `announceFingerprint_`'s field delimiter, invisible as anything but a space in an editor/terminal) will hash differently from the raw file and get permanently, silently blocked unless the build step replicates that same normalization. Only found because AC required a real-browser live-verification pass on SIT before PROD — a unit test hashing the raw file bytes would have passed while the live page failed to load at all.
- CSP `connect-src` is enforced against every hop of a redirect chain, not just the initial request URL — this app's `callApi` doPost observably 302s through `script.googleusercontent.com`, so a `connect-src` scoped to the literal GAS `/exec` origin alone (the issue's original wording) would have silently broken every check-in/signup call under the policy; the redirect target has to be included explicitly.

## 2026-08-19 21:15:00
_session 7891b164 · v3 · 08-19_

### Objective 1: Deliver PLAN-08-19.md Stage 2 — data-entry quality hardening
Rationale: Stage 2 aims to close two independent, no-gate hardening gaps flagged as cheap and high-value to ship immediately: signup could be completed with no team and no WHO/WHAT/HOW goals (the "McLovin abandoned at identify" case), and a re-pasted Slack Link on a Bonus Tracker entry was silently dropped by the sheet's own dedup with no visible signal to the PAX (F3Go30-fkox's confirmed-intentional dedup, minus the missing UX). Grouped into one session per the plan's own reasoning: both are small enough that per-session startup cost dominates over their separate file contexts.
Outcome [user-facing]: Signup's info step (static-pages/src/index.html) now blocks Continue until a team is chosen and WHO/WHAT/HOW are all filled in, with inline reasons explaining why each matters; a new ℹ️ button opens a team-guidance splash reusing the announcement splash's overlay/modal CSS (F3Go30-g9bi) rather than a new component. A reused Slack Link on a bonus entry now surfaces a plain-language "won't add points" notice, both at save time and as a persistent list badge, mirroring the existing weekly-cap notice (F3Go30-833s.16).
Outcome [developer-facing]: New script/BonusTypes.js function `annotateBonusEntryDuplicateLinkStatus_` mirrors `annotateBonusEntryCountStatus_`'s shape and is wired into dashboardWebapp.js's `handleBonusList_` alongside it — detection only, no scoring change. Added test/test_bonus_types.js (+5 cases) and extended the existing real-source extraction harness in test/test_bonus_save_notice.js (+3 cases) rather than writing a new harness. Extended tests/playwright/static-signup.spec.js with a live-SIT test covering both validation messages and the info modal.
Outcome [internal]: Deployed to SIT (build 2.5.0.2); full `npm test` and the extended static-signup.spec.js suite (4/4) verified green against live SIT. docs/CHANGELOG.md §Unreleased got two PAX-facing bullets. PLAN-08-19.md Stage 2 checklist checked off with a completion write-up and a handoff note to Stage 3 (reuse the same overlay CSS for FAQ/How-it-Works). bd: F3Go30-9u68 and F3Go30-6faz.1 closed; F3Go30-6faz epic closed (last child done).
Open: A pre-existing, unrelated uncommitted diff (script/bonusWebapp.js / test/test_bonus_webapp.js, the F3Go30-6faz.2 retry-guard) was sitting in the working tree at session start and predates this session's work — flagged to the user but not touched or committed. Nothing in this session was committed to git; that was called out as still needing an explicit go-ahead.

## 2026-08-20 19:59:30
_session a7c27eb9-2cb9-4419-a45e-9bf8d3033708 · v3 · 08-20_

### Objective 1: Execute PLAN-08-20.md Stage 1 — dashboard/identify tail-latency investigation
Rationale: The two prime PAX-facing latency risks flagged in a 2026-08-05 investigation (`bd memories F3Go30-perf-checkin-dashboard-eval`) — `checkinWebapp.dashboard` spiking 8-32s with 25-30s unexplained, and `checkinWebapp.identify` losing 2-4s in `buildMonthNavigationPayload_dw_`'s two unthrottled full-sheet reads — had a concrete, already-written next-steps list that was never executed. PLAN-08-20.md flagged this as the highest-priority debt in the plan, ahead of any feature work, since it's a live risk today rather than backlog. Filed `F3Go30-bopt` before starting per the project's create-before-coding convention.
Outcome [developer-facing]: `script/dashboardWebapp.js` gained timing instrumentation: `buildMonthNavigationPayload_dw_` now logs `checkinWebapp.buildMonthNavigationPayload.timing` with `trackerDbReadMs`/`paxDbReadMs` split; `handleCheckinDashboard_`'s existing `checkinWebapp.dashboard` log gained `renderMs` plus `bonusEntriesMs`/`bonusCacheHit`/`historyEntriesMs`/`rosterLoopMs` sub-splits. Deployed to SIT (build 2.5.0.5), exercised against a real SIT PAX via `tools/callWebapp.js`, and cross-checked against Axiom (`tools/query_axiom.py`) — both original hypotheses confirmed with live numbers: `buildMonthNavigationPayload_dw_` (990ms in one sample) closes the `identify` gap down to ~300ms unaccounted (from 2-4s), and `renderMs` is now fully explained by its three new sub-timers in every sample taken, with a cold bonus-cache miss as the dominant driver of a slow render rather than an unmeasured black box. `npm test` stayed green (no test asserted the log payload's exact shape). Filed the actual fix — capping/paginating the two unthrottled reads — as its own issue, `F3Go30-m732`, linked `related` to `F3Go30-bopt`, per the plan's own scoping rule against folding an uncertain-size fix into the investigation session.
Outcome [internal]: `bd memories F3Go30-perf-checkin-dashboard-eval` updated in place with the confirmed findings (superseding the 2026-08-05 "NEXT STEPS," including a `tools/query_axiom.py` `--where` syntax correction: use bare `execId == ...`, not `data.execId`). `F3Go30-bopt` closed. PLAN-08-20.md Stage 1 checklist checked off with Completed/residual-gap/Handoff notes; the Handoff flags `F3Go30-m732` as a direct, evidence-backed input to `F3Go30-uz9e.5`'s design (Stage 4).
Open: the two stalled (`status:0`, no-response) requests from the original 2026-08-05 HAR capture were not reproduced this session — the re-test was scripted single-shot API calls, not an equivalent real-browser session, so absence of reproduction is weak evidence. Root cause remains unknown; left open in the memory for whoever picks it up next, rather than closed out as resolved.

## 2026-08-20 14:11:46
_session 43761129-1e01-45f9-af24-0cef12a0c93b · v3 · 08-20_

### Objective 1: Close out PLAN-08-20 Stage 2 — test-infrastructure hygiene (hard-coded dates, idea triage, extraction-helper duplication, fixture pollution)
Rationale: Four independent testing-infrastructure debt items were batched into one stage because none touch production code and none overlap in files — closing them together clears standing debt (one issue open 30+ days per `bd stale`) before September's date boundary and before the fixture-pollution bug silently stops exercising its own validation path.
Outcome [developer-facing]: `F3Go30-493w` audited and closed — every hard-coded 2026-08-*/August-2026 date literal across 15 test files, plus every bare `new Date()` call, confirmed to already route through an injected `today`/`nowIso`/`contextDate` parameter or a `Date.now()`-relative offset (both month-agnostic); no code changes required, findings recorded on the issue.
Outcome [developer-facing]: `F3Go30-31w5` re-triaged — its one idea was already fully implemented (`.1`/`.2`, both closed); confirmation note added, issue stays open as the collection point.
Outcome [developer-facing]: Filed and closed `F3Go30-lem7` — folded `readStaticPage_`/`extractTransportBlock_`/`extractShowApiError_`, hand-copied across four test files, onto the existing shared `test/helpers/staticPageExtract.js`, removing the dead `fs`/`path` requires each copy left behind.
Outcome [user-facing]: Filed and closed `F3Go30-vecg` — `tests/playwright/static-signup.spec.js`'s blocked-validation test now generates a fresh random PAX identity per run instead of a fixed constant, so a completed save on SIT can no longer make the test silently stop exercising the validation path it exists to check; live-verified with 4/4 passing against SIT (`npm run test:static-signup`).
Outcome [developer-facing]: `npm test` re-run clean across all suites after every change.

## 2026-08-20 21:15:00
_session 22b1bd03 · v3 · 08-20_

### Objective 1: Close the Slice 3 design gap blocking PaxProfile migration from starting
Rationale: F3Go30-uz9e.5's own notes required the §6 backfill-mechanics design pass to run in an Opus session before implementation could be scheduled — "this is the one slice that changes the primary write path AND requires a real data migration of live PAX data... the open questions are design tradeoffs with irreversible consequences, not implementation work." Spawned an Opus subagent to close it, grounded against the live code (advancePaxHistoryDay_/Entry_, getPaxHistoryWindowValues_/paxHistoryWindowMatchesTracker_, handleTrackerEdit_, upsertPaxDbRow_) rather than the doc alone.
Outcome [developer-facing]: docs/pax-data-model-and-contract.md gained §5.3 (phase model + PaxProfile.mode cutover flag, backfill batching against the verified 6-min Apps Script execution limit, checkpoint-after-write resume, four-check verification including a formula-vs-code derived-score check, dual-write/rollback with a one-month Tracker mirror + immutable pre-cutover snapshot, and the post-migration manual-edit answer: onEdit becomes a one-way Tracker→PaxProfile write-through, invariant 6 restated to invert at cutover). §6's two blocking items marked resolved; §5.3.7 gives an 8-step implementation checklist. F3Go30-uz9e.5 updated with the summary.
Open: Slice 3 implementation itself (starting at §5.3.7 step 1: inert PaxProfile/staging/report schema + mode flag defaulting to 'off') is unstarted — flagged as ordinary work, no longer needing Opus.

### Objective 2: Settle whether client-side board consolidation must wait for the PaxProfile migration
Rationale: F3Go30-os03 explicitly deferred its own design ("reassess again whether to wait for Slice 3... once Slice 2's shape is in hand") pending this reassessment. Verified against the live client (static-pages/src/index.html) that today's fragmentation is real — state.monthCache is populated only by the dashboard RPC, identify writes a separate localStorage snapshot, monthGrid has no cache at all — and confirmed docs/pax-data-model-and-contract.md §3.2 makes Slice 3 contract-neutral by design, so a client-side redesign against today's shapes carries no rework risk once Slice 3 lands.
Outcome [developer-facing]: Decision recorded on F3Go30-os03: proceed against today's three-RPC contract, don't wait for Slice 3. Answered the issue's own open question #1 (fetch ownership) — dashboard's response becomes the canonical per-monthKey board record, identify/monthGrid patch into the same slot, the localStorage snapshot stays a separate offline-bootstrap layer. Implementation of the client refactor itself remains open work.

### Objective 3: Fix the settings-menu feedback email arriving with no message or star rating  [accreted]
Transition: user reported the bug mid-session ("i am not seeing in the email the feedback message, or the star rating"); investigated immediately rather than deferring since it's a live, user-visible defect.
