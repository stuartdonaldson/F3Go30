...
Outcome [developer-facing]: ADR-018 — the static origin is the primary signup front end, implemented as a step inside index.html; SignupApp.html demoted to zero-install fallback, with its retirement explicitly left undecided to keep the record to one decision.
