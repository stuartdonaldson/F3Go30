#!/usr/bin/env node
/**
 * F3Go30 deployment config. The pipeline itself lives in the shared `gas-deploy` package; this
 * file is only what is specific to this project. See that package's README and
 * best-practices/gas-deployment/RECOMMENDATION.md for internals.
 *
 *   pnpm run deploy:sit | deploy:prod
 *   node tools/manage-deployments.js --summary --env sit    # read-only, deploys nothing
 *
 * Public env vocabulary is sit/prod; internal target keys are test/template (the two script
 * projects) — envAliases maps between them. The retired `month` target (ADR-010, F3Go30-shsx) is
 * deliberately absent; its local.settings.json keys survive as historical residue.
 */
...
const config = {
  ...
  // --summary only: flags live-vs-local divergence. Display-only — never read back on deploy.
  readLocalVersion: () => {
  ...
  // Regenerates the "How it Works" panels from docs/Go30-Intro.md (F3Go30-e3co). Must be IN the
  // push, hence prePush.
  prePush: [
    { name: 'Sync How it Works panels', run: () => execSync('node tools/sync-how-it-works.js', { stdio: 'inherit', cwd: ROOT }) },
  ],
  postDeploy: [
    // Wipes PaxCache + the layout/full-roster CacheService blobs so code just pushed that changed
    // cache shape can't trip over a stale entry (F3Go30-x2vd). Must run AFTER the deploy —
    // wiping first just lets the still-live old code repopulate a stale entry.
    ...
    // Bounds trigger growth to the prev/current/next-month window (2026-08-20 SIT quota
    // incident: nothing swept except on demand, so triggers crept toward the per-script quota).
