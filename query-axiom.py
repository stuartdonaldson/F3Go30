...
from local.settings.json.example).
...
Examples:
    ...
    python query-axiom.py --name autoGenerateNextMonthTracker
    python query-axiom.py --where "data.spreadsheetId == '1AAE...'"
...
def _load_settings() -> dict:
    if not _SETTINGS_PATH.exists():
        raise FileNotFoundError(
            f"{_SETTINGS_PATH} not found. Copy local.settings.json.example and fill in your Axiom values."
        )
...
def main() -> int:
    ...
    parser.add_argument("--where", help="Raw APL `where` expression, e.g. \"data.spreadsheetId == 'xyz'\"")
